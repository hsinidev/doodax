...
function getTools() {
    ...
    // We need to make the content valid JSON to parse it, or just eval it (risky but easy for this script)
    // Since it contains backticks for icons, JSON.parse won't work directly if we just wrap it.
    // However, we can use a regex to extract just the names and roles, which is safer and sufficient.
    const tools = [];
...
async function generatePost(tool, type, index) {
    ...
    try {
        ...
        post.relatedTools = []; // We can fill this later or leave empty
...
async function main() {
    ...
    let posts = [];
    // Try to load existing generated posts to resume if needed
    ...
    // Types of articles to rotate through
    ...
    for (let i = 0; i < tools.length; i++) {
        ...
        // Generate 2 posts per tool to reach ~200
        for (let j = 0; j < 2; j++) {
            if (posts.length >= targetPosts) break;
